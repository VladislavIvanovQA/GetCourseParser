...
1. Убедитесь, что **GetCourseDownloader** запущен в терминале.
2. Откройте урок GetCourse в браузере (URL вида `…/pl/teach/control/lesson/view?id=…`).
3. **Включите видео** в плеере (хотя бы на несколько секунд) — иначе ссылки HLS могут не появиться.
4. Клик по иконке расширения → **Подключиться** (токен подставится с `config.json`).
5. **+ В очередь** — добавить урок; можно открыть другие уроки в новых вкладках и добавить ещё.
6. Popup можно закрыть — очередь обрабатывается в фоне.

Результат: `dist/GetCourseDownloader/downloads/<название урока>/`
